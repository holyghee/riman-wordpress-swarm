        ...
        wp.element.createElement('div', { className: 'riman-hero-slider-editor-placeholder' },
          ...
          wp.element.createElement(InspectorControls, null,
            ...
            wp.element.createElement(PanelBody, { title: __('Kategorien', 'riman'), initialOpen: false },
              wp.element.createElement(TextControl, {
                label: __('Nur diese Slugs (Komma-getrennt)', 'riman'),
                value: include,
                onChange: (v)=> { setInclude(v); const arr = v.split(',').map(s=>s.trim()).filter(Boolean); setAttributes({ includeCategories: arr }); }
              })
            )
          )
        )
